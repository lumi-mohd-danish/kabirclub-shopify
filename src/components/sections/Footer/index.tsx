...
const Footer = () => {
  return (
    // The footer is one of the design's ink bands. The ground stays on the
    // <footer> so the zari rule below can sit flush against the page above it;
    // everything under the rule is a real `.section-band`, which is what brings
    // the ink ground, the paper text, the film grain (3.5% inline feTurbulence,
    // clipped to the band by its own `isolation: isolate`) and — the point of
    // this fix — the system's 80/128/160 vertical rhythm. The hand-rolled
    // 64/80/96 ladder that used to live on the inner container was short of the
    // token at every breakpoint, so the footer sat tighter than every other
    // band on the site.
    <footer className="w-full bg-ink-900">
